...
public:                     			//下一行为普通构造函数声明
	Date(int y = 2007, int m = 1, int d = 1);
	void Print(Student &stu)       //成员函数，其形式参数为类Student的对象引用
	{
		cout << stu.specialty << endl;               //先显示学生对象的专业名称
		cout << year << "-" << month << "-" << day << endl;   //再显示本类当前对象代表的日期
	}
...
public:
	Student(char  *pSpec);
	~Student();
	friend void Date::Print( Student &);   //此为声明友元成员的语句，Date::不可少
};
Student::Student(char *pSpec)                  //Student类的构造函数定义
{
	if (pSpec)
...
int main()
{
	Student zhang("computer");
	Date Birthday(1990, 10, 17);
	Birthday.Print(zhang);       	      //通过Date类对象Birthday调用函数也同时显示对象zhang的信息
	system("pause");
	return 0;
}